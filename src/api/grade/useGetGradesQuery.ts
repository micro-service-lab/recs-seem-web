...
export const useGetGradesQuery = (params: GradeQueryParam) => {
  const { data, status, isPending } = useSuspenseQuery({
    queryKey: [organizationQueryKey.grade.list, params],
    queryFn: async () => {
      return axios
        .get<
          | DefaultResponse
        ...
        >(ORGANIZATION_ENDPOINTS.grade.get, {
          params,
        })
        .then((res) => res.data);
    },
